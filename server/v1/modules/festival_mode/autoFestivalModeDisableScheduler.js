...
export const runFestivalModeAutoDisableJob = async () => {
  ...
  if (expiredEnabledConfigs.length > 0) {
    ...
    console.log("[FESTIVAL MODE] Auto-disable check complete: no expired enabled config");
  }
};

export const initializeFestivalModeAutoDisableScheduler = () => {
  agenda.define(
    JOB_NAME,
    async () => {
      try {
  ...
  // Every day at 03:10 AM IST (kept away from existing midnight/early-morning jobs).
