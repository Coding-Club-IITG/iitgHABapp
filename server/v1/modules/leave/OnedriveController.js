...
const uploadFilesToOnedrive = async (req, res, next) => {
  ...
  const uniqueMiddleName = `-${req.user._id}-${timeStamp}-${uniqueSuffix}-`;
    ...
    if(proofDocument) {
      req.uploadedDocuments.proofDocument = await uploadToOnedrive( proofDocument.buffer, proofDocument.mimetype, `proofdocument${uniqueMiddleName}${proofDocument.originalname}`, LEAVE_FOLDER_ID,res);
    }
    if(leaveDocument) {
      req.uploadedDocuments.leaveDocument = await uploadToOnedrive( leaveDocument.buffer, leaveDocument.mimetype, `proofdocument${uniqueMiddleName}${leaveDocument.originalname}`, LEAVE_FOLDER_ID,res);
    }
