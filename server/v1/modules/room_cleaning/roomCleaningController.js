...
/**
 * DEV/TEST ONLY: POST /api/room-cleaning/dev/auto-test
 * Auto-assigns a cleaner and finalizes all non-cancelled bookings for
 * the authenticated user so the frontend can be tested autonomously.
 *
 * Body (optional):
 *  - finalizeAs: "Cleaned" | "CouldNotBeCleaned" (default "Cleaned")
 *  - reason: string (required when finalizeAs = CouldNotBeCleaned)
 */
const devAutoTest = async (req, res) => {
  try {
    if (!req.user?._id) {
      return res.status(401).json({ message: "User not authenticated" });
    }

    const { finalizeAs = "Cleaned", reason = "Student Did Not Respond" } =
      req.body || {};
    ...
    if (bookings.length === 0) {
      return res
        .status(404)
        .json({ message: "No active bookings found to auto-test" });
    }

    const results = [];
    for (const booking of bookings) {
      // Find or create a cleaner for this hostel
      let cleaner = await RcCleaner.findOne({
        hostelId: booking.hostelId,
      }).lean();
      if (!cleaner) {
        cleaner = await RcCleaner.create({
          hostelId: booking.hostelId,
          name: "Test Cleaner",
          slots: ["A", "B", "C", "D"],
        });
      ...
      sendNotificationToUser(
        booking.userId,
        "Room Cleaning Allotted",
        `Your room cleaning has been successfully allotted to ${cleaner.name}.`,
        { redirectType: "room_cleaning" },
      ).catch(() => {});
      ...
      const updateFields = {
        status: finalizeAs,
        statusFinalizedAt: now,
      };
      if (finalizeAs === "CouldNotBeCleaned") {
      ...
      if (finalizeAs === "Cleaned") {
        ...
        sendNotificationToUser(
          booking.userId,
          "Room Cleaning Update",
          `Your room cleaning could not be completed. Reason: ${reason}.`,
          { redirectType: "room_cleaning" },
        ).catch(() => {});
      }
      ...
      results.push({
        ...
        finalizedAs: finalizeAs,
      });
    }

    return res.status(200).json({
      message: `Auto-test complete: ${results.length} booking(s) assigned and finalized`,
      results,
    });
  } catch (err) {
    console.error("devAutoTest error:", err);
    return res.status(500).json({
      message: "Auto-test failed",
      error: String(err?.message || err),
    });
  }
};
